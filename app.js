...
app.post("/sellerlogin", function (req, res) {
    ...
    sellerData.find().then(function (seller) {
        ...
        for (i = 0; i < seller.length; i++) {
            if (
                sellerdata.email == seller[i].email &&
                sellerdata.password == seller[i].password
            ) {
                var seller = {
...
app.get("/get-seller-products/:sellerid", function (req, res) {
    ...
    productData.find({ sellerid: sellerid }).then(function (product) {
        if (product.length == 0) {
            res.status(401).json({
...
app.get("/get-seller-product/:_id", function (req, res) {
    ...
    productData.findOne({ _id: id }).then(function (product) {
        res.send(product);
    });
});
...
app.get("/deleteProduct/:_id", function (req, res) {
    ...
    productData.findByIdAndDelete({ _id: _id }).then(() => {
...
app.get("/get-cart-product/:_id", function (req, res) {
    ...
    productData.findOne({ _id: _id }).then(function (product) {
        res.send(product);
    });
});
...
app.get("/cartproducts/:userid", function (req, res) {
    ...
    cartData.find({ userid: userid }).then(function (cartproduct) {
...
app.get("/deletecartproduct/:_id", function (req, res) {
    ...
    cartData.findByIdAndDelete({ _id: _id }).then(() => {
        console.log("product deleted");
        res.send();
    });
});
